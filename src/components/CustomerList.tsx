...
export function CustomerList({ initialCustomers }: { initialCustomers: Customer[] }) {
  ...
  return (
    <div className="flex flex-col h-full max-w-md mx-auto bg-gray-50 min-h-screen">
      <div className="sticky top-0 z-10 bg-white border-b-4 border-gray-900 px-4 py-4 shadow-md">
      ...
      <div className="p-3 space-y-3 pb-28">
        {filtered.length === 0 && (
          <div className="text-center py-20 text-gray-500 font-bold text-xl uppercase tracking-widest">No customers found</div>
        )}
        {filtered.map(c => (
          <Link key={c.id} href={`/customers/${c.id}`} className="block bg-white shadow-sm border-2 border-gray-200 p-5 active:bg-gray-100 transition-colors">
